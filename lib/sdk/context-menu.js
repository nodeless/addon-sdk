...
const USERPOSITIONED_ITEM_CLASS = "addon-context-menu-item-userpositioned";
...
let LabelledItem = Class({
  ...
  get userPosition() {
    return internal(this).options.userPosition;
  }
});
...
let ItemContainer = Class({
  initialize: function initialize() {
    ...
    internal(this).userPositionedChildren = [];
  },
  ...
  addItem: function addItem(item) {
    ...
    let childrenWithoutUserPos = this.onlySystemPosChildren;
    if (childrenWithoutUserPos.length > 0)
      after = childrenWithoutUserPos[childrenWithoutUserPos.length - 1];

    if (item.userPosition)
      internal(this).userPositionedChildren.push(item);
    children.push(item);
  ...
  get onlySystemPosChildren() {
    return internal(this).children.filter(
            function(x){return internal(this).userPositionedChildren.indexOf(x) === -1;}.bind(this));
  },
...
let MenuWrapper = Class({
  ...
  get userPositionedItems() {
    return this.contextMenu.querySelectorAll("." + USERPOSITIONED_ITEM_CLASS);
  },
  ...
  populate: function populate(menu) {
    for (let item of internal(menu).userPositionedChildren) {
      this.createItem(item, null);
    }
    let systemPosChildren = menu.onlySystemPosChildren;
    for (let i = 0; i < systemPosChildren.length; i++) {
      let item = systemPosChildren[i];
      let after = i === 0 ? null : systemPosChildren[i - 1];
  ...
  insertIntoXUL: function insertIntoXUL(item, node, after) {
    ...
    let userBeforeElement = this.window.document.getElementById(item.userPosition);
    if (userBeforeElement) {
        before = userBeforeElement;
    }
    else if (after) {
  ...
  updateXULClass: function updateXULClass(xulNode) {
    if (xulNode.getAttribute('userPosition')) {
      xulNode.classList.add(USERPOSITIONED_ITEM_CLASS);
      return;
    }
  ...
  createItem: function createItem(item, after) {
    ...
    if (item instanceof LabelledItem) {
      ...
      if (item.userPosition)
        xulNode.setAttribute("userPosition", item.userPosition);
